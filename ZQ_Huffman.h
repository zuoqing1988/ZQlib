...
namespace ZQ
{
	...
	ZQ_HuffmanNode::ZQ_HuffmanNode():index(-1),freq(0),leftChild(0),rightChild(0)
	{

	}

	ZQ_HuffmanNode::~ZQ_HuffmanNode()
	{

	}

	void ZQ_HuffmanNode::ReleaseNode(ZQ_HuffmanNode** node)
	{
		if(*node == 0)
			return ;
	...
	bool ZQ_HuffmanCode::ImportFromBitStream(const unsigned char *bitStream, unsigned long byteLen)
	{
		_clear();
	...
	bool ZQ_HuffmanCode::ExportToBitStream(unsigned char ** bitStream,unsigned long * byteLen)
	{
	...
	ZQ_Huffman::ZQ_Huffman():length(0),data(0),huffmanTree(0)
	{
		for(int i = 0;i < 256;i++)
			hist[i] = 0;
	}

	ZQ_Huffman::~ZQ_Huffman()
	{
		_destroyHuffmanTree();
	}

	void ZQ_Huffman::SetDataLength(unsigned long length)
	{
		this->length = length;
	}

	void ZQ_Huffman::SetData(const unsigned char* data)
	{
		this->data = data;
	}

	bool ZQ_Huffman::HuffmanEncode()
	{
		if(!_calHist())
			return false;
	...
	ZQ_HuffmanCode* ZQ_Huffman::ExportHuffmanCode()
	{
	...
	void ZQ_Huffman::_clearHist()
	{
		for(int i = 0;i < 256;i++)
			hist[i] = 0;
	}

	bool ZQ_Huffman::_buildHuffmanTree()
	{
		_destroyHuffmanTree();
		int nodeNum = 0;
	...
	void ZQ_Huffman::_destroyHuffmanTree()
	{
		ZQ_HuffmanNode::ReleaseNode(&(this->huffmanTree));
	}

	void ZQ_Huffman::PrintHist()
	{
		for(int i = 0;i < 256;i++)
		{
			if(hist[i] != 0)
	...
	ZQ_HuffmanEncoder::ZQ_HuffmanEncoder(ZQ_HuffmanCode &m_huffcode)
	{
		for(int i = 0;i < 256;i++)
		{
			this->code[i] = 0;
	...
	ZQ_HuffmanEncoder::~ZQ_HuffmanEncoder()
	{
		for(int i = 0;i < 256;i++)
		{
			if(this->code[i] != 0)
	...
	bool ZQ_HuffmanEncoder::EncodeOneByte(unsigned char value, char out[], unsigned long *out_len)
	{
	...
	ZQ_HuffmanDecoder::ZQ_HuffmanDecoder(ZQ_HuffmanCode &m_huffcode)
	{
	...
	ZQ_HuffmanDecoder::~ZQ_HuffmanDecoder()
	{
		ZQ_HuffmanNode::ReleaseNode(&(this->huffmanTree));

	}

	bool ZQ_HuffmanDecoder::DecoderBitStream(const unsigned char *bitStream, unsigned long bytelen, unsigned long offsetbitlen, unsigned char **out, unsigned long *out_len)
	{
		ZQ_BitStream* m_bitstream = new ZQ_BitStream(bytelen);
		unsigned long reallen;
		m_bitstream->SetData(bitStream,bytelen,&reallen);
	...
	bool ZQ_HuffmanEndec::ZQ_HuffmanEncodeFile(const char infile[], const char outfile[])
	{
	...
	bool ZQ_HuffmanEndec::ZQ_HuffmanDecodeFile(const char infile[], const char outfile[], char originalName[])
	{
		FILE* in = fopen(infile,"rb");
		if(in == 0)
			return false;
	...
	bool ZQ_HuffmanEndec::ZQ_HuffmanEncodeByteStream(const unsigned char* input, unsigned long inlen, unsigned char** output, unsigned long* outlen)
	{
	...
	bool ZQ_HuffmanEndec::ZQ_HuffmanDecodeByteStream(const unsigned char* input, unsigned long inlen, unsigned char** output, unsigned long* outlen)
	{
