...
int test_binocalib_with_known_intrinsic()
{
	...
	{
		T left_rT[6];
		ZQ_CameraCalibrationBino::_get_left_rT_from_right_rT_fun(right_rT + vv * 6, right_to_left_rT, left_rT);
		ZQ_CameraProjection::project_points_fun(num_of_pts, X3 + num_of_pts * 3 * vv, left_rT, left_fc, left_cc, left_kc, left_alpha_c, left_X2 + num_of_pts * 2 * vv, zAxis_in);
		ZQ_CameraProjection::project_points_fun(num_of_pts, X3 + num_of_pts * 3 * vv, right_rT + vv * 6, right_fc, right_cc, right_kc, right_alpha_c, right_X2 + num_of_pts * 2 * vv, zAxis_in);
	}
	...
	int max_iter = 30;
	clock_t t1 = clock();
	if (!ZQ_CameraCalibrationBino::CalibrateBinocularCamera<T>(n_views, num_of_pts, X3, left_X2, right_X2, out_right_to_left_rT, out_right_rT, left_fc, left_cc, left_kc, left_alpha_c,
...
int test_binocalib()
{
	...
	int max_iter = 30;
	clock_t t1 = clock();
	if (!ZQ_CameraCalibrationBino::CalibrateBinocularCamera2<T>(n_views, num_of_pts, X3, left_X2, right_X2, out_right_to_left_rT, out_right_rT, out_left_fc, out_left_cc, out_left_kc, out_left_alpha_c,
		out_right_fc, out_right_cc, out_right_kc, out_right_alpha_c, ZQ_CameraCalibrationMono::CALIB_F2_C_ALPHA_K5, zAxis_in, 0, 0, 0, 0, max_iter, true, true))
	{
		printf("failed \n");
		return -1;
	}
