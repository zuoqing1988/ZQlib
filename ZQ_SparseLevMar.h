...
namespace ZQ
{
	...
	bool ZQ_SparseLevMar::ZQ_SparseLevMar_Der(
		...
		int max_iter,													
		const ZQ::ZQ_SparseLevMarOptions &opts, ZQ::ZQ_SparseLevMarReturnInfos &info, const void *data)
	{
		...
		{	
			...
			while(1)
			{
				...
				int pcg_it;
				if(!ZQ_PCGSolver::PCG_sparse_unsquare(aug_jac_mat,e,Dx0,opts.pcg_solver_max_iter,opts.pcg_solver_tol,Dx,pcg_it,false))
				{
