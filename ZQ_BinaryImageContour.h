...
namespace ZQ
{
	...
	bool ZQ_BinaryImageContour::GetBinaryImageContour(const bool* image, int width, int height, std::vector<std::vector<ZQ_Vec2D>>& contours)
	{
		if(image == 0)
			return false;
	...
	bool ZQ_BinaryImageContour::GetBinaryImageMaxContour(const bool *image, int width, int height, std::vector<ZQ_Vec2D> &contour)
	{
	...
	bool ZQ_BinaryImageContour::_findSeedAndSeedFilling(const bool *image, bool *used_flag, int width, int height, int &x, int &y)
	{
		bool flag = false;
		for(int i = 0;i < height;i++)
		{
	...
	void ZQ_BinaryImageContour::_findContour(const bool *image, int width, int height, int x, int y, std::vector<ZQ_Vec2D> &contour)
	{
		int cur_x = x;
		int cur_y = y;
