...
namespace ZQ
{
	namespace ZQ_ImageIO
	{
		/*save tga image only support 3 channel image, data are in range [0,1] and arranged [R G B]*/
		template<class T>
		bool writeTGAimage(const T* data, const int width, const int height, const int nChannels, const char* filename);

		template<class T>
		bool writePFMimage(const T* data, const int width, const int height, const int nChannels, const char* filename);

		/*load save gray image or rgb image*/
		template<class T>
		bool loadImage(ZQ_DImage<T>& im, const char* filename, int iscolor = 0);

		template<class T>
		bool saveImage(const ZQ_DImage<T>& im, const char* filename);

		/*save flow field, color_type is 0 or 1*/
		template<class T>
		IplImage* SaveFlowToColorImage(const ZQ_DImage<T>& u, const ZQ_DImage<T>& v,  bool user_input, float max_rad, int wheelSize,int color_type, bool display = false);

		template<class T>
		IplImage* SaveFlowToColorImage(const ZQ_DImage<T>& u1, const ZQ_DImage<T>& v1, const ZQ_DImage<T>& u2, const ZQ_DImage<T>& v2,  
			bool user_input, float max_rad, int wheelSize,int color_type, bool display = false);

		/*handle flow field*/
		template<class T>
		T* FlowToColor0(const int width, const int height, const T* pU, const T* pV, bool user_input_max_rad = false, float user_max_rad = 10.0, bool display = false);

		template<class T>
		void MakeColorWheel(int& ncol, T*& colorwheel);

		template<class T>
		T* FlowToColor1(const int width, const int height, const T* pU, const T* pV, bool user_input_max_rad = false, float user_max_rad = 10.0, bool display = false);

		template<class T>
		void HSV2RGB(const int npxiels, const T* hsv_image, T* rgb_image);

		IplImage* MakeColorWheelImage0(int N, bool user_input, float max_rad);

		IplImage* MakeColorWheelImage1(int N, bool user_input, float max_rad);
		...
		template<class T>
		bool ZQ_ImageIO::loadImage(ZQ_DImage<T>& im, const char *filename, int iscolor)
		{
		...
		template<class T>
		bool ZQ_ImageIO::saveImage(const ZQ_DImage<T>& im, const char* filename)
		{
		...
		template<class T>
		IplImage* ZQ_ImageIO::SaveFlowToColorImage(const ZQ_DImage<T>& u, const ZQ_DImage<T>& v, bool user_input, float max_rad, int wheelSize,int color_type, bool display)
		{
			int width = u.width();
			int height = v.height();
		...
		template<class T>
		IplImage* ZQ_ImageIO::SaveFlowToColorImage(const ZQ_DImage<T>& u1, const ZQ_DImage<T>& v1, const ZQ_DImage<T>& u2, const ZQ_DImage<T>& v2, 
			bool user_input, float max_rad, int wheelSize,int color_type, bool display )
		{
			int width1 = u1.width();
		...
		template<class T>
		T* ZQ_ImageIO::FlowToColor0(const int w, const int h, const T* uu, const T* vv, bool user_input_max_rad, float user_max_rad, bool display)
		{
			T* img = new T[w*h*3];
			T* u = new T[w*h];
			T* v = new T[w*h];
		...
		template<class T>
		void ZQ_ImageIO::MakeColorWheel(int& ncols, T*& colorwheel)
		{
			int RY = 15;
			int YG = 6;
			int GC = 4;
		...
		template<class T>
		T* ZQ_ImageIO::FlowToColor1(const int w, const int h, const T* uu, const T* vv, bool user_input_max_rad, float user_max_rad, bool display)
		{
		...
		template<class T>
		void ZQ_ImageIO::HSV2RGB(const int npixels, const T* hsv_image, T* rgb_image)
		{
			T* h = new T[npixels];
			T* s = new T[npixels];
			T* v = new T[npixels];
	...
	IplImage* ZQ_ImageIO::MakeColorWheelImage0(int N, bool user_input, float max_rad)
	{
		float cx = (N-1)*0.5;
		float cy = (N-1)*0.5;
	...
	IplImage* ZQ_ImageIO::MakeColorWheelImage1(int N, bool user_input, float max_rad)
	{
		float cx = (N-1)*0.5;
		float cy = (N-1)*0.5;
		float* u = new float[N*N];
