...
namespace ZQ
{
	...
	public:
		...
		void SetVolumeDensityScale(float densityScale = 1.0f);
		void SetOpacityScale(float opacityScale = 1.0f);
		void SetOpacityThreshold(float opacityThreshold = 0.98);
	...
	ZQ_CPURaycasting::ZQ_CPURaycasting()
	{
		/*window's width and height, focus is the center*/
		width = 400;
		height = 400;
	...
	ZQ_CPURaycasting::IntersectRes ZQ_CPURaycasting::IntersectBox(const ZQ_Ray3D r, const ZQ_Vec3D& boxmin, const ZQ_Vec3D& boxmax)
	{
	...
	float ZQ_CPURaycasting::MiddleValue(float x, float y, float z)
	{
		if(x <= y && y <= z)
			return y;
		else if(x <= z && z <= y)
	...
	void ZQ_CPURaycasting::SetWindowSize(unsigned int width, unsigned int height)
	{
		this->width = width;
		this->height = height;
	}

	void ZQ_CPURaycasting::SetInnerPara(float cx, float cy, float fx, float fy)
	{
		this->center_x = cx;
		this->center_y = cy;
		this->focal_x = fx;
	...
	void ZQ_CPURaycasting::SetViewMatrix(const float* view_mat)
	{
		memcpy(this->view_matrix,view_mat,sizeof(float)*16);
	}

	void ZQ_CPURaycasting::SetWorldMatrix(const float* world_mat)
	{
		memcpy(this->world_matrix,world_mat,sizeof(float)*16);
	}

	void ZQ_CPURaycasting::SetVolumeData(const float* data, unsigned int xsize, unsigned int ysize, unsigned int zsize)
	{
		this->xsize = xsize;
		this->ysize = ysize;
		this->zsize = zsize;
	...
	void ZQ_CPURaycasting::SetVolumeDensityScale(float densityScale)
	{
		this->densityScale = densityScale;
	}

	void ZQ_CPURaycasting::SetOpacityScale(float opacityScale)
	{
		this->opacityScale = opacityScale;
	}

	void ZQ_CPURaycasting::SetOpacityThreshold(float opacityThreshold)
	{
		this->opacityThreshold = opacityThreshold;
	}

	void ZQ_CPURaycasting::SetVolumeBoundingBox(const ZQ_Vec3D& boxmin, const ZQ_Vec3D& boxmax)
	{
		this->boundingBoxMin = boxmin;
		this->boundingBoxMax = boxmax;
	}

	bool ZQ_CPURaycasting::RenderToBuffer(float* buffer,int max_steps,ColorFormat fmt)
	{
	...
	int ZQ_CPURaycasting::_castRay(const float* c_invMv,unsigned int w, unsigned h, unsigned int max_step, float& r, float& g, float& b, float& a)
	{
		r = g = b = a =  0.0f;
