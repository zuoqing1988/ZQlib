...
public:
	...
	bool handle_args(const int argc, const char** argv)
	{
		...
		while(k < argc)
		{
			...
			else if(_strcmpi(argv[k],"sample_mode") == 0)
			{
				k++;
				if (k >= argc)
				{
					printf("need the value for sample_mode\n");
					return false;
				}
				sample_mode = atoi(argv[k]);
			}
...
void test(int argc, const char** argv)
{
	...
	if(!ZQ_BlendTwoImages::BlendTwoImages<T>(
		width,height,nChannels,img1.data(),img2.data(),u.data(),v.data(),weight1,out_img.data(),opt.skip,opt.iterations,opt.number_of_neighbor,opt.radius_scale,opt.sample_mode,opt.blend_mode))
	{
