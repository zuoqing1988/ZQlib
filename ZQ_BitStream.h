...
namespace ZQ
{
	...
	public:
		/*be sure bytelen >= 1*/
		ZQ_BitStream(unsigned long bytelen);
		~ZQ_BitStream();
	...
	ZQ_BitStream::ZQ_BitStream(unsigned long bytelen):maxlen(bytelen)
	{
	...
	ZQ_BitStream::~ZQ_BitStream()
	{
		free(this->data);
		this->data = 0;
	}

	void ZQ_BitStream::BackToStart()
	{
		this->index = 0;
		this->offset = 0;
	}

	void ZQ_BitStream::Reset()
	{
		this->index = 0;
		this->offset = 0;
		this->realBitlen = 0;
	...
	bool ZQ_BitStream::GetBit(bool *value)
	{
	...
	bool ZQ_BitStream::GetByte(unsigned char *value)
	{
	...
	bool ZQ_BitStream::AddBit(bool value)
	{
	...
	bool ZQ_BitStream::AddByte(unsigned char value)
	{
		unsigned long oldIndex = this->index;
		unsigned long oldoffset = this->offset;
	...
	bool ZQ_BitStream::SetData(const unsigned char *data, unsigned long len, unsigned long *reallen)
	{
		if(data == 0)
			return false;
		if(len <= this->maxlen)
	...
	bool ZQ_BitStream::ExportData(unsigned char **data, unsigned long* bytelen, unsigned long* bitlen)
	{
