...
#include "ZQ_MultiCamCalibration.h"
#include <stdio.h>
#include <time.h>
#include <stdlib.h>
using namespace ZQ;
	...
	const T* X3, int n_pts, T* X2, bool zAxis_in)
{
	...
	if (!ZQ_CameraCalibration::project_points_fun(n_pts, X3, mv_rT, fc_cc_alpha_kc, fc_cc_alpha_kc + 2, fc_cc_alpha_kc + 5, fc_cc_alpha_kc[4], X2, zAxis_in))
	{
		return false;
	}
...
int test_multicalib_with_known_intrinsic()
{
	...
	if (!ZQ_MultiCamCalibration::CalibrateMultiCamera<T>(n_checkboard, num_of_pts, n_cams, X3, all_X2, visible_num, visible_offset, visible_idx, fc_cc_alpha_kc, zAxis_in,
