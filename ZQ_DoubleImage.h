...
namespace ZQ
{
	...
	public:
		...
		void imfilter_h(ZQ_DImage& image, T* filter, int fsize) const;
		void imfilter_v(ZQ_DImage& image, T* filter, int fsize) const;
		void imfilter_hv(ZQ_DImage& image, const T* hfilter, int hfsize, const T* vfilter, int vfsize) const;
		...
		void assemble(const ZQ_DImage& u, const ZQ_DImage& v);
	...
	bool ZQ_DImage<T>::imresize(float ratio)
	{
		...
		ZQ_ImageProcessing::ResizeImage(pData,pDstData,imWidth,imHeight,nChannels,dstWidth,dstHeight);
	...
	bool ZQ_DImage<T>::imresize(int dstWidth, int dstHeight)
	{
		...
		ZQ_ImageProcessing::ResizeImage(pData,foo.data(),imWidth,imHeight,nChannels,dstWidth,dstHeight);
		copyData(foo);
		return true;
	}
	...
	void ZQ_DImage<T>::imresize(ZQ_DImage<T>& result, float ratio) const
	{
		...
		if(result.width() != dstWidth || result.height() != dstHeight || result.nchannels() != nChannels)
			...
			result.reset();
		ZQ_ImageProcessing::ResizeImage(pData,result.data(),imWidth,imHeight,nChannels,dstWidth,dstHeight);
	}
	...
	bool ZQ_DImage<T>::imresizeBicubic(float ratio)
	{
		...
		ZQ_ImageProcessing::ResizeImageBicubic(pData,pDstData,imWidth,imHeight,nChannels,dstWidth,dstHeight);
	...
	bool ZQ_DImage<T>::imresizeBicubic(int dstWidth, int dstHeight)
	{
		...
		ZQ_DImage<T> foo(dstWidth,dstHeight,nChannels);
		ZQ_ImageProcessing::ResizeImageBicubic(pData,foo.data(),imWidth,imHeight,nChannels,dstWidth,dstHeight);
		copyData(foo);
		return true;
	}
	...
	void ZQ_DImage<T>::imresizeBicubic(ZQ_DImage<T>& result, float ratio) const
	{
		...
		if(result.width() != dstWidth || result.height() != dstHeight || result.nchannels() != nChannels)
			...
			result.reset();
		ZQ_ImageProcessing::ResizeImageBicubic(pData,result.data(),imWidth,imHeight,nChannels,dstWidth,dstHeight);
	}
	...
	void ZQ_DImage<T>::dx(ZQ_DImage<T>& result, bool isAdvancedFilter) const
	{
		...
		else
		{
			...
			for(int i = 0; i < 5;i++)
				xFilter[i] /= 12;
			ZQ_ImageProcessing::Hfiltering(pData,data,imWidth,imHeight,nChannels,xFilter,2);
		}
	}
	...
	void ZQ_DImage<T>::dy(ZQ_DImage<T>& result, bool isAdvancedFilter) const
	{
		...
		else
		{
			...
			for(int i = 0;i < 5;i++)
				yFilter[i] /= 12;
			ZQ_ImageProcessing::Vfiltering(pData,data,imWidth,imHeight,nChannels,yFilter,2);
		}
	}
	...
	template<class T>
	void ZQ_DImage<T>::imfilter_h(ZQ_DImage<T>& image, T* filter, int fsize) const
	{
		if(matchDimension(image) == false)
			image.allocate(imWidth,imHeight,nChannels);
		ZQ_ImageProcessing::Hfiltering(pData,image.data(),imWidth,imHeight,nChannels,filter,fsize);
	}


	template<class T>
	void ZQ_DImage<T>::imfilter_v(ZQ_DImage<T>& image, T* filter, int fsize) const
	{
		if(matchDimension(image) == false)
			image.allocate(imWidth,imHeight,nChannels);
		ZQ_ImageProcessing::Vfiltering(pData,image.data(),imWidth,imHeight,nChannels,filter,fsize);
	}



	template<class T>
	void ZQ_DImage<T>::imfilter_hv(ZQ_DImage<T> &image, const T *hfilter, int hfsize, const T *vfilter, int vfsize) const
	{
		if(matchDimension(image) == false)
			image.allocate(imWidth,imHeight,nChannels);
		T* pTempBuffer = new T[nElements];
		ZQ_ImageProcessing::Hfiltering(pData,pTempBuffer,imWidth,imHeight,nChannels,hfilter,hfsize);
		ZQ_ImageProcessing::Vfiltering(pTempBuffer,image.data(),imWidth,imHeight,nChannels,vfilter,vfsize);
		delete []pTempBuffer;
	}
	...
	void ZQ_DImage<T>::assemble(const ZQ_DImage<T> &u, const  ZQ_DImage<T> &v)
	{
		...
		if(nchannles != 1 || v.matchDimension(u) == false)
			return ;

		allocate(width,height,2);
		for(int i = 0;i < nPixels;i++)
		{
			pData[i*2+0] = u.pData[i];
			pData[i*2+1] = v.pData[i];
		}
	}
	...
	void ZQ_DImage<T>::warpImage(ZQ_DImage<T> &output, const ZQ_DImage<T>& u, const ZQ_DImage<T>& v) const
	{
		if(!output.matchDimension(*this))
			output.allocate(*this);
		ZQ_ImageProcessing::WarpImage(output.data(),pData,u.data(),v.data(),imWidth,imHeight,nChannels);
	}
